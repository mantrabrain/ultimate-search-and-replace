...
class USAR_AJAX {
	...
	public function init() {
		...
		add_action( 'init', array( $this, 'do_usar_ajax' ), 2 );
	...
	public static function get_endpoint() {
		return esc_url_raw( get_admin_url() . 'tools.php?page=ultimate-search-and-replace&usar-ajax=' );
	}

	/**
	 * Set USAR AJAX constant and headers.
	 * @access public
	 */
	public function define_ajax() {

		if ( isset( $_GET['usar-ajax'] ) && ! empty( $_GET['usar-ajax'] ) ) {
	 ...
	 * @access public
	 */
	public function do_usar_ajax() {
		global $wp_query;

		if ( isset( $_GET['usar-ajax'] ) && ! empty( $_GET['usar-ajax'] ) ) {
			$wp_query->set( 'usar-ajax', sanitize_text_field( $_GET['usar-ajax'] ) );
		}

		if ( $action = $wp_query->get( 'usar-ajax' ) ) {
			do_action( 'usar_ajax_' . sanitize_text_field( $action ) );
			die();
		}
	}
	...
	public function add_ajax_actions() {
		...
		foreach ( $actions as $action ) {
			add_action( 'usar_ajax_' . $action, array( $this, $action ) );
		}
	}
	...
	public function process_search_replace() {
		...
		if ( ! check_admin_referer( 'usar_ajax_nonce', 'usar_ajax_nonce' ) ) {
			return;
		}

		$args = array();

		if ( isset( $_POST['usar_data'] ) ) {
			parse_str( $_POST['usar_data'], $args );
		}


		// Initialize the DB class.
		$db 				= new USAR_DB();
		$step 				= isset( $_POST['usar_step' ] ) ? absint( $_POST['usar_step'] ) : 0;
		$page 				= isset( $_POST['usar_page'] ) ? absint( $_POST['usar_page'] ) : 0;
		...
		if ( isset( $args['select_tables'][$step] ) ) {
			...
			if ( isset( $args['select_tables'][$step] ) ) {
				$message = sprintf(
					__( 'Processing table %d of %d: %s', 'ultimate-search-and-replace' ),
					$step + 1,
		...
		$result = array(
			...
			'url' 				=> get_admin_url() . 'tools.php?page=ultimate-search-and-replace&tab=usar_search_replace&result=true',
			'usar_data' 			=> build_query( $args )
		);
	...
	/**
	 * Helper function for assembling the USAR Results.
	 * @access public
	...
	public function append_report( $table, $report, $args ) {
		...
		$results = get_transient( 'usar_results' ) ? get_transient( 'usar_results') : array();
		...
		if ( ! set_transient( 'usar_results', $results, DAY_IN_SECONDS ) ) {
			return false;
		}
