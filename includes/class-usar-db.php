...
class USAR_DB {
	...
	public static function get_sizes() {
		...
		if ( is_array( $tables ) && ! empty( $tables ) ) {
			...
			foreach ( $tables as $table ) {
				...
				$sizes[$table['Name']] = sprintf( __( '(%s MB)', 'ultimate-search-and-replace' ), $size );
			}
	...
	public function get_page_size() {
		$page_size = get_option( 'usar_page_size' ) ? get_option( 'usar_page_size' ) : 20000;
	...
	public function srdb( $table, $page, $args ) {
		...
		$args['search_for'] 	= str_replace( '#USAR_BACKSLASH#', '\\', $args['search_for'] );
		$args['replace_with'] 	= str_replace( '#USAR_BACKSLASH#', '\\', $args['replace_with'] );
		...
		foreach ( $data as $row ) {
			...
			foreach( $columns as $column ) {
				...
				if ( $this->wpdb->options === $table ) {
					...
					// Skip any USAR options as they may contain the search field.
					...
					if ( isset( $update_later ) && true === $update_later ) {
						...
						if ( $edited_data != $data_to_fix ) {
							...
							update_option( 'usar_update_site_url', $edited_data );
							continue;
						}
					}

					if ( '_transient_usar_results' === $data_to_fix || 'usar_profiles' === $data_to_fix || 'usar_update_site_url' === $data_to_fix ) {
						$should_skip = true;
					}
			...
			if ( $args['dry_run'] === 'on' ) {
				...
				if ( ! $result ) {
					$table_report['errors'][] = sprintf( __( 'Error updating row: %d.', 'ultimate-search-and-replace' ), $current_row );
				} else {
					$table_report['updates']++;
				}
	...
	public function maybe_update_site_url() {
		$option = get_option( 'usar_update_site_url' );

		if ( $option ) {
			update_option( 'siteurl', $option );
			delete_option( 'usar_update_site_url' );
			return true;
		}
